...
getDataCovid_US <- function(date = NULL, FromTo = NULL, by = c("day", "week", "month"),
                            level = c("states"), download_dir = NULL)
{
  ...
  if(is.null(date) & is.null(FromTo)){
    ...
    file <- grep(paste("US/RData/",level, "/us_",level, sep = ""), file, value = TRUE, fixed = TRUE)
    ...
    by <- match.arg(by, choices = eval(formals(getDataCovid_US)$by))
